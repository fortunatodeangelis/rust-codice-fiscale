...
fn main() {
    ...
    let born_year: i32 = date_check[2..4].parse::<i32>().unwrap();
    let mut born_month: usize = date_check[5..7].parse::<usize>().unwrap();
    born_month = born_month -1;
    let mut born_day: i32 = date_check[8..10].parse::<i32>().unwrap();
    ...
    result.push_str(&format_lastname(lastname_check, &consonants, &vocals)); 
    result.push_str(&format_name(name_check, &consonants, &vocals)); 
